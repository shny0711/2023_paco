...
def record(t):
    ...
    for i in range(args.record_num):
        ...
        while not done:
            ...
            h_list.append(high_action)
            w_list.append((1+high_action)/2)

            plt.close()
            
            wandb.log({"h":high_action,f"tmp/w-{t}-{i}":(1+high_action)/2, "step": step})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cls", default="HL_SAC", choices=["SAC", "MLPSAC", "HL_SAC"])
    ...
    wandb.init(entity="kai-shunya", project="hl_sac_1次元_data", config=args)
